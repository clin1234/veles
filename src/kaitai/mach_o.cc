// This is a generated file! Please edit source .ksy file and use kaitai-struct-compiler to rebuild

#include "kaitai/mach_o.h"

#include <iostream>
#include <fstream>

mach_o_t::mach_o_t(kaitai::kstream *p_io, kaitai::kstruct *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = this;
    veles_obj = m__io->startChunk("mach_o");
    m__io->pushName("magic");
    m_magic = static_cast<mach_o_t::magic_type_t>(m__io->read_u4be());
    m__io->popName();
    m_header = new mach_header_t(m__io, this, m__root);
    int l_load_commands = header()->ncmds();
    m_load_commands = new std::vector<load_command_t*>();
    m_load_commands->reserve(l_load_commands);
    for (int i = 0; i < l_load_commands; i++) {
        m_load_commands->push_back(new load_command_t(m__io, this, m__root));
    }
    m__io->endChunk();
}

mach_o_t::~mach_o_t() {
    delete m_header;
    for (std::vector<load_command_t*>::iterator it = m_load_commands->begin(); it != m_load_commands->end(); ++it) {
        delete *it;
    }
    delete m_load_commands;
}

mach_o_t::rpath_command_t::rpath_command_t(kaitai::kstream *p_io, mach_o_t::load_command_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    veles_obj = m__io->startChunk("rpath_command");
    m__io->pushName("path_offset");
    m_path_offset = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("path");
    m_path = m__io->read_strz("utf-8", 0, false, true, true);
    m__io->popName();
    m__io->endChunk();
}

mach_o_t::rpath_command_t::~rpath_command_t() {
}

mach_o_t::uleb128_t::uleb128_t(kaitai::kstream *p_io, kaitai::kstruct *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    f_value = false;
    m_b1 = m__io->read_u1();
    n_b2 = true;
    if ((b1() & 128)) {
        n_b2 = false;
        m_b2 = m__io->read_u1();
    }
    n_b3 = true;
    if ((b2() & 128)) {
        n_b3 = false;
        m_b3 = m__io->read_u1();
    }
    n_b4 = true;
    if ((b3() & 128)) {
        n_b4 = false;
        m_b4 = m__io->read_u1();
    }
    n_b5 = true;
    if ((b4() & 128)) {
        n_b5 = false;
        m_b5 = m__io->read_u1();
    }
    n_b6 = true;
    if ((b5() & 128)) {
        n_b6 = false;
        m_b6 = m__io->read_u1();
    }
    n_b7 = true;
    if ((b6() & 128)) {
        n_b7 = false;
        m_b7 = m__io->read_u1();
    }
    n_b8 = true;
    if ((b7() & 128)) {
        n_b8 = false;
        m_b8 = m__io->read_u1();
    }
    n_b9 = true;
    if ((b8() & 128)) {
        n_b9 = false;
        m_b9 = m__io->read_u1();
    }
    n_b10 = true;
    if ((b9() & 128)) {
        n_b10 = false;
        m_b10 = m__io->read_u1();
    }
}

mach_o_t::uleb128_t::~uleb128_t() {
}

int32_t mach_o_t::uleb128_t::value() {
    if (f_value)
        return m_value;
    m_value = ((kaitai::kstream::mod(b1(), 128) << 0) + ((b1() & 128) == 0) ? (0) : (((kaitai::kstream::mod(b2(), 128) << 7) + ((b2() & 128) == 0) ? (0) : (((kaitai::kstream::mod(b3(), 128) << 14) + ((b3() & 128) == 0) ? (0) : (((kaitai::kstream::mod(b4(), 128) << 21) + ((b4() & 128) == 0) ? (0) : (((kaitai::kstream::mod(b5(), 128) << 28) + ((b5() & 128) == 0) ? (0) : (((kaitai::kstream::mod(b6(), 128) << 35) + ((b6() & 128) == 0) ? (0) : (((kaitai::kstream::mod(b7(), 128) << 42) + ((b7() & 128) == 0) ? (0) : (((kaitai::kstream::mod(b8(), 128) << 49) + ((b8() & 128) == 0) ? (0) : (((kaitai::kstream::mod(b9(), 128) << 56) + ((b8() & 128) == 0) ? (0) : ((kaitai::kstream::mod(b10(), 128) << 63)))))))))))))))))));
    f_value = true;
    return m_value;
}

mach_o_t::source_version_command_t::source_version_command_t(kaitai::kstream *p_io, mach_o_t::load_command_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    veles_obj = m__io->startChunk("source_version_command");
    m__io->pushName("version");
    m_version = m__io->read_u8le();
    m__io->popName();
    m__io->endChunk();
}

mach_o_t::source_version_command_t::~source_version_command_t() {
}

mach_o_t::cs_blob_t::cs_blob_t(kaitai::kstream *p_io, kaitai::kstruct *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_magic = static_cast<mach_o_t::cs_blob_t::cs_magic_t>(m__io->read_u4be());
    m_length = m__io->read_u4be();
    switch (magic()) {
    case CS_MAGIC_DETACHED_SIGNATURE:
        m__raw_body = m__io->read_bytes((length() - 8));
        m__io__raw_body = new kaitai::kstream(m__raw_body);
        m_body = new super_blob_t(m__io__raw_body, this, m__root);
        break;
    case CS_MAGIC_EMBEDDED_SIGNATURE:
        m__raw_body = m__io->read_bytes((length() - 8));
        m__io__raw_body = new kaitai::kstream(m__raw_body);
        m_body = new super_blob_t(m__io__raw_body, this, m__root);
        break;
    case CS_MAGIC_ENTITLEMENT:
        m__raw_body = m__io->read_bytes((length() - 8));
        m__io__raw_body = new kaitai::kstream(m__raw_body);
        m_body = new entitlement_t(m__io__raw_body, this, m__root);
        break;
    case CS_MAGIC_BLOB_WRAPPER:
        m__raw_body = m__io->read_bytes((length() - 8));
        m__io__raw_body = new kaitai::kstream(m__raw_body);
        m_body = new blob_wrapper_t(m__io__raw_body, this, m__root);
        break;
    case CS_MAGIC_REQUIREMENT:
        m__raw_body = m__io->read_bytes((length() - 8));
        m__io__raw_body = new kaitai::kstream(m__raw_body);
        m_body = new requirement_t(m__io__raw_body, this, m__root);
        break;
    case CS_MAGIC_CODE_DIRECTORY:
        m__raw_body = m__io->read_bytes((length() - 8));
        m__io__raw_body = new kaitai::kstream(m__raw_body);
        m_body = new code_directory_t(m__io__raw_body, this, m__root);
        break;
    case CS_MAGIC_REQUIREMENTS:
        m__raw_body = m__io->read_bytes((length() - 8));
        m__io__raw_body = new kaitai::kstream(m__raw_body);
        m_body = new entitlements_t(m__io__raw_body, this, m__root);
        break;
    default:
        m__raw_body = m__io->read_bytes((length() - 8));
        break;
    }
}

mach_o_t::cs_blob_t::~cs_blob_t() {
}

mach_o_t::cs_blob_t::entitlement_t::entitlement_t(kaitai::kstream *p_io, mach_o_t::cs_blob_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_data = m__io->read_bytes_full();
}

mach_o_t::cs_blob_t::entitlement_t::~entitlement_t() {
}

mach_o_t::cs_blob_t::code_directory_t::code_directory_t(kaitai::kstream *p_io, mach_o_t::cs_blob_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    f_ident = false;
    f_team_id = false;
    f_hashes = false;
    m_version = m__io->read_u4be();
    m_flags = m__io->read_u4be();
    m_hash_offset = m__io->read_u4be();
    m_ident_offset = m__io->read_u4be();
    m_n_special_slots = m__io->read_u4be();
    m_n_code_slots = m__io->read_u4be();
    m_code_limit = m__io->read_u4be();
    m_hash_size = m__io->read_u1();
    m_hash_type = m__io->read_u1();
    m_spare1 = m__io->read_u1();
    m_page_size = m__io->read_u1();
    m_spare2 = m__io->read_u4be();
    n_scatter_offset = true;
    if (version() >= 131328) {
        n_scatter_offset = false;
        m_scatter_offset = m__io->read_u4be();
    }
    n_team_id_offset = true;
    if (version() >= 131584) {
        n_team_id_offset = false;
        m_team_id_offset = m__io->read_u4be();
    }
}

mach_o_t::cs_blob_t::code_directory_t::~code_directory_t() {
    delete m_hashes;
}

std::string mach_o_t::cs_blob_t::code_directory_t::ident() {
    if (f_ident)
        return m_ident;
    std::streampos _pos = m__io->pos();
    m__io->seek((ident_offset() - 8));
    m_ident = m__io->read_strz("utf-8", 0, false, true, true);
    m__io->seek(_pos);
    f_ident = true;
    return m_ident;
}

std::string mach_o_t::cs_blob_t::code_directory_t::team_id() {
    if (f_team_id)
        return m_team_id;
    std::streampos _pos = m__io->pos();
    m__io->seek((team_id_offset() - 8));
    m_team_id = m__io->read_strz("utf-8", 0, false, true, true);
    m__io->seek(_pos);
    f_team_id = true;
    return m_team_id;
}

std::vector<std::string>* mach_o_t::cs_blob_t::code_directory_t::hashes() {
    if (f_hashes)
        return m_hashes;
    std::streampos _pos = m__io->pos();
    m__io->seek(((hash_offset() - 8) - (hash_size() * n_special_slots())));
    int l_hashes = (n_special_slots() + n_code_slots());
    m_hashes = new std::vector<std::string>();
    m_hashes->reserve(l_hashes);
    for (int i = 0; i < l_hashes; i++) {
        m_hashes->push_back(m__io->read_bytes(hash_size()));
    }
    m__io->seek(_pos);
    f_hashes = true;
    return m_hashes;
}

mach_o_t::cs_blob_t::entitlements_blob_index_t::entitlements_blob_index_t(kaitai::kstream *p_io, mach_o_t::cs_blob_t::entitlements_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    f_value = false;
    m_type = static_cast<mach_o_t::cs_blob_t::entitlements_blob_index_t::requirement_type_t>(m__io->read_u4be());
    m_offset = m__io->read_u4be();
}

mach_o_t::cs_blob_t::entitlements_blob_index_t::~entitlements_blob_index_t() {
    if (f_value) {
        delete m_value;
    }
}

mach_o_t::cs_blob_t* mach_o_t::cs_blob_t::entitlements_blob_index_t::value() {
    if (f_value)
        return m_value;
    std::streampos _pos = m__io->pos();
    m__io->seek((offset() - 8));
    m_value = new cs_blob_t(m__io, this, m__root);
    m__io->seek(_pos);
    f_value = true;
    return m_value;
}

mach_o_t::cs_blob_t::data_t::data_t(kaitai::kstream *p_io, kaitai::kstruct *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_length = m__io->read_u4be();
    m_value = m__io->read_bytes(length());
    m_padding = m__io->read_bytes((4 - (length() & 3)));
}

mach_o_t::cs_blob_t::data_t::~data_t() {
}

mach_o_t::cs_blob_t::super_blob_t::super_blob_t(kaitai::kstream *p_io, mach_o_t::cs_blob_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_count = m__io->read_u4be();
    int l_blobs = count();
    m_blobs = new std::vector<blob_index_t*>();
    m_blobs->reserve(l_blobs);
    for (int i = 0; i < l_blobs; i++) {
        m_blobs->push_back(new blob_index_t(m__io, this, m__root));
    }
}

mach_o_t::cs_blob_t::super_blob_t::~super_blob_t() {
    for (std::vector<blob_index_t*>::iterator it = m_blobs->begin(); it != m_blobs->end(); ++it) {
        delete *it;
    }
    delete m_blobs;
}

mach_o_t::cs_blob_t::expr_t::expr_t(kaitai::kstream *p_io, kaitai::kstruct *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_op = static_cast<mach_o_t::cs_blob_t::expr_t::op_enum_t>(m__io->read_u4be());
    switch (op()) {
    case OP_ENUM_CERT_GENERIC:
        m_data = new cert_generic_expr_t(m__io, this, m__root);
        break;
    case OP_ENUM_APPLE_GENERIC_ANCHOR:
        m_data = new apple_generic_anchor_expr_t(m__io, this, m__root);
        break;
    case OP_ENUM_INFO_KEY_FIELD:
        m_data = new info_key_field_expr_t(m__io, this, m__root);
        break;
    case OP_ENUM_AND_OP:
        m_data = new and_expr_t(m__io, this, m__root);
        break;
    case OP_ENUM_ANCHOR_HASH:
        m_data = new anchor_hash_expr_t(m__io, this, m__root);
        break;
    case OP_ENUM_INFO_KEY_VALUE:
        m_data = new data_t(m__io, this, m__root);
        break;
    case OP_ENUM_OR_OP:
        m_data = new or_expr_t(m__io, this, m__root);
        break;
    case OP_ENUM_TRUSTED_CERT:
        m_data = new cert_slot_expr_t(m__io, this, m__root);
        break;
    case OP_ENUM_NOT_OP:
        m_data = new expr_t(m__io, this, m__root);
        break;
    case OP_ENUM_IDENT:
        m_data = new ident_expr_t(m__io, this, m__root);
        break;
    case OP_ENUM_CERT_FIELD:
        m_data = new cert_field_expr_t(m__io, this, m__root);
        break;
    case OP_ENUM_ENTITLEMENT_FIELD:
        m_data = new entitlement_field_expr_t(m__io, this, m__root);
        break;
    case OP_ENUM_CD_HASH:
        m_data = new data_t(m__io, this, m__root);
        break;
    default:
        break;
    }
}

mach_o_t::cs_blob_t::expr_t::~expr_t() {
}

mach_o_t::cs_blob_t::expr_t::info_key_field_expr_t::info_key_field_expr_t(kaitai::kstream *p_io, mach_o_t::cs_blob_t::expr_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_data = new data_t(m__io, this, m__root);
    m_match = new match_t(m__io, this, m__root);
}

mach_o_t::cs_blob_t::expr_t::info_key_field_expr_t::~info_key_field_expr_t() {
    delete m_data;
    delete m_match;
}

mach_o_t::cs_blob_t::expr_t::cert_slot_expr_t::cert_slot_expr_t(kaitai::kstream *p_io, mach_o_t::cs_blob_t::expr_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_value = static_cast<mach_o_t::cs_blob_t::expr_t::cert_slot_t>(m__io->read_u4be());
}

mach_o_t::cs_blob_t::expr_t::cert_slot_expr_t::~cert_slot_expr_t() {
}

mach_o_t::cs_blob_t::expr_t::cert_generic_expr_t::cert_generic_expr_t(kaitai::kstream *p_io, mach_o_t::cs_blob_t::expr_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_cert_slot = static_cast<mach_o_t::cs_blob_t::expr_t::cert_slot_t>(m__io->read_u4be());
    m_data = new data_t(m__io, this, m__root);
    m_match = new match_t(m__io, this, m__root);
}

mach_o_t::cs_blob_t::expr_t::cert_generic_expr_t::~cert_generic_expr_t() {
    delete m_data;
    delete m_match;
}

mach_o_t::cs_blob_t::expr_t::ident_expr_t::ident_expr_t(kaitai::kstream *p_io, mach_o_t::cs_blob_t::expr_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_identifier = new data_t(m__io, this, m__root);
}

mach_o_t::cs_blob_t::expr_t::ident_expr_t::~ident_expr_t() {
    delete m_identifier;
}

mach_o_t::cs_blob_t::expr_t::cert_field_expr_t::cert_field_expr_t(kaitai::kstream *p_io, mach_o_t::cs_blob_t::expr_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_cert_slot = static_cast<mach_o_t::cs_blob_t::expr_t::cert_slot_t>(m__io->read_u4be());
    m_data = new data_t(m__io, this, m__root);
    m_match = new match_t(m__io, this, m__root);
}

mach_o_t::cs_blob_t::expr_t::cert_field_expr_t::~cert_field_expr_t() {
    delete m_data;
    delete m_match;
}

mach_o_t::cs_blob_t::expr_t::anchor_hash_expr_t::anchor_hash_expr_t(kaitai::kstream *p_io, mach_o_t::cs_blob_t::expr_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_cert_slot = static_cast<mach_o_t::cs_blob_t::expr_t::cert_slot_t>(m__io->read_u4be());
    m_data = new data_t(m__io, this, m__root);
}

mach_o_t::cs_blob_t::expr_t::anchor_hash_expr_t::~anchor_hash_expr_t() {
    delete m_data;
}

mach_o_t::cs_blob_t::expr_t::apple_generic_anchor_expr_t::apple_generic_anchor_expr_t(kaitai::kstream *p_io, mach_o_t::cs_blob_t::expr_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    f_value = false;
}

mach_o_t::cs_blob_t::expr_t::apple_generic_anchor_expr_t::~apple_generic_anchor_expr_t() {
}

std::string mach_o_t::cs_blob_t::expr_t::apple_generic_anchor_expr_t::value() {
    if (f_value)
        return m_value;
    m_value = std::string("anchor apple generic");
    f_value = true;
    return m_value;
}

mach_o_t::cs_blob_t::expr_t::entitlement_field_expr_t::entitlement_field_expr_t(kaitai::kstream *p_io, mach_o_t::cs_blob_t::expr_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_data = new data_t(m__io, this, m__root);
    m_match = new match_t(m__io, this, m__root);
}

mach_o_t::cs_blob_t::expr_t::entitlement_field_expr_t::~entitlement_field_expr_t() {
    delete m_data;
    delete m_match;
}

mach_o_t::cs_blob_t::expr_t::and_expr_t::and_expr_t(kaitai::kstream *p_io, mach_o_t::cs_blob_t::expr_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_left = new expr_t(m__io, this, m__root);
    m_right = new expr_t(m__io, this, m__root);
}

mach_o_t::cs_blob_t::expr_t::and_expr_t::~and_expr_t() {
    delete m_left;
    delete m_right;
}

mach_o_t::cs_blob_t::expr_t::or_expr_t::or_expr_t(kaitai::kstream *p_io, mach_o_t::cs_blob_t::expr_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_left = new expr_t(m__io, this, m__root);
    m_right = new expr_t(m__io, this, m__root);
}

mach_o_t::cs_blob_t::expr_t::or_expr_t::~or_expr_t() {
    delete m_left;
    delete m_right;
}

mach_o_t::cs_blob_t::blob_index_t::blob_index_t(kaitai::kstream *p_io, mach_o_t::cs_blob_t::super_blob_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    f_blob = false;
    m_type = static_cast<mach_o_t::cs_blob_t::blob_index_t::csslot_type_t>(m__io->read_u4be());
    m_offset = m__io->read_u4be();
}

mach_o_t::cs_blob_t::blob_index_t::~blob_index_t() {
    if (f_blob) {
        delete m__io__raw_blob;
        delete m_blob;
    }
}

mach_o_t::cs_blob_t* mach_o_t::cs_blob_t::blob_index_t::blob() {
    if (f_blob)
        return m_blob;
    kaitai::kstream *io = _parent()->_io();
    std::streampos _pos = io->pos();
    io->seek((offset() - 8));
    m__raw_blob = io->read_bytes_full();
    m__io__raw_blob = new kaitai::kstream(m__raw_blob);
    m_blob = new cs_blob_t(m__io__raw_blob, this, m__root);
    io->seek(_pos);
    f_blob = true;
    return m_blob;
}

mach_o_t::cs_blob_t::match_t::match_t(kaitai::kstream *p_io, kaitai::kstruct *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_match_op = static_cast<mach_o_t::cs_blob_t::match_t::op_t>(m__io->read_u4be());
    n_data = true;
    if (match_op() != OP_EXISTS) {
        n_data = false;
        m_data = new data_t(m__io, this, m__root);
    }
}

mach_o_t::cs_blob_t::match_t::~match_t() {
    if (!n_data) {
        delete m_data;
    }
}

mach_o_t::cs_blob_t::requirement_t::requirement_t(kaitai::kstream *p_io, mach_o_t::cs_blob_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_kind = m__io->read_u4be();
    m_expr = new expr_t(m__io, this, m__root);
}

mach_o_t::cs_blob_t::requirement_t::~requirement_t() {
    delete m_expr;
}

mach_o_t::cs_blob_t::blob_wrapper_t::blob_wrapper_t(kaitai::kstream *p_io, mach_o_t::cs_blob_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_data = m__io->read_bytes_full();
}

mach_o_t::cs_blob_t::blob_wrapper_t::~blob_wrapper_t() {
}

mach_o_t::cs_blob_t::entitlements_t::entitlements_t(kaitai::kstream *p_io, mach_o_t::cs_blob_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_count = m__io->read_u4be();
    int l_items = count();
    m_items = new std::vector<entitlements_blob_index_t*>();
    m_items->reserve(l_items);
    for (int i = 0; i < l_items; i++) {
        m_items->push_back(new entitlements_blob_index_t(m__io, this, m__root));
    }
}

mach_o_t::cs_blob_t::entitlements_t::~entitlements_t() {
    for (std::vector<entitlements_blob_index_t*>::iterator it = m_items->begin(); it != m_items->end(); ++it) {
        delete *it;
    }
    delete m_items;
}

mach_o_t::segment_command_64_t::segment_command_64_t(kaitai::kstream *p_io, mach_o_t::load_command_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    veles_obj = m__io->startChunk("segment_command_64");
    m__io->pushName("segname");
    m_segname = m__io->read_str_byte_limit(16, "ascii");
    m__io->popName();
    m__io->pushName("vmaddr");
    m_vmaddr = m__io->read_u8le();
    m__io->popName();
    m__io->pushName("vmsize");
    m_vmsize = m__io->read_u8le();
    m__io->popName();
    m__io->pushName("fileoff");
    m_fileoff = m__io->read_u8le();
    m__io->popName();
    m__io->pushName("filesize");
    m_filesize = m__io->read_u8le();
    m__io->popName();
    m__io->pushName("maxprot");
    m_maxprot = static_cast<mach_o_t::vm_prot_t>(m__io->read_u4le());
    m__io->popName();
    m__io->pushName("initprot");
    m_initprot = static_cast<mach_o_t::vm_prot_t>(m__io->read_u4le());
    m__io->popName();
    m__io->pushName("nsects");
    m_nsects = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("flags");
    m_flags = m__io->read_u4le();
    m__io->popName();
    int l_sections = nsects();
    m_sections = new std::vector<section_64_t*>();
    m_sections->reserve(l_sections);
    for (int i = 0; i < l_sections; i++) {
        m_sections->push_back(new section_64_t(m__io, this, m__root));
    }
    m__io->endChunk();
}

mach_o_t::segment_command_64_t::~segment_command_64_t() {
    for (std::vector<section_64_t*>::iterator it = m_sections->begin(); it != m_sections->end(); ++it) {
        delete *it;
    }
    delete m_sections;
}

mach_o_t::segment_command_64_t::section_64_t::section_64_t(kaitai::kstream *p_io, mach_o_t::segment_command_64_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    f_data = false;
    veles_obj = m__io->startChunk("section_64");
    m__io->pushName("sect_name");
    m_sect_name = m__io->read_str_byte_limit(16, "ascii");
    m__io->popName();
    m__io->pushName("seg_name");
    m_seg_name = m__io->read_str_byte_limit(16, "ascii");
    m__io->popName();
    m__io->pushName("addr");
    m_addr = m__io->read_u8le();
    m__io->popName();
    m__io->pushName("size");
    m_size = m__io->read_u8le();
    m__io->popName();
    m__io->pushName("offset");
    m_offset = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("align");
    m_align = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("reloff");
    m_reloff = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("nreloc");
    m_nreloc = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("flags");
    m_flags = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("reserved1");
    m_reserved1 = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("reserved2");
    m_reserved2 = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("reserved3");
    m_reserved3 = m__io->read_u4le();
    m__io->popName();
    m__io->endChunk();
}

mach_o_t::segment_command_64_t::section_64_t::~section_64_t() {
}

mach_o_t::segment_command_64_t::section_64_t::cf_string_list_t::cf_string_list_t(kaitai::kstream *p_io, mach_o_t::segment_command_64_t::section_64_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_items = new std::vector<cf_string_t*>();
    while (!m__io->is_eof()) {
        m_items->push_back(new cf_string_t(m__io, this, m__root));
    }
}

mach_o_t::segment_command_64_t::section_64_t::cf_string_list_t::~cf_string_list_t() {
    for (std::vector<cf_string_t*>::iterator it = m_items->begin(); it != m_items->end(); ++it) {
        delete *it;
    }
    delete m_items;
}

mach_o_t::segment_command_64_t::section_64_t::cf_string_t::cf_string_t(kaitai::kstream *p_io, mach_o_t::segment_command_64_t::section_64_t::cf_string_list_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_isa = m__io->read_u8le();
    m_info = m__io->read_u8le();
    m_data = m__io->read_u8le();
    m_length = m__io->read_u8le();
}

mach_o_t::segment_command_64_t::section_64_t::cf_string_t::~cf_string_t() {
}

mach_o_t::segment_command_64_t::section_64_t::eh_frame_item_t::eh_frame_item_t(kaitai::kstream *p_io, mach_o_t::segment_command_64_t::section_64_t::eh_frame_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_length = m__io->read_u4le();
    n_length64 = true;
    if (length() == 4294967295) {
        n_length64 = false;
        m_length64 = m__io->read_u8le();
    }
    m_id = m__io->read_u4le();
    n_body = true;
    if (length() > 0) {
        n_body = false;
        switch (id()) {
        case 0:
            m__raw_body = m__io->read_bytes((length() - 4));
            m__io__raw_body = new kaitai::kstream(m__raw_body);
            m_body = new cie_t(m__io__raw_body, this, m__root);
            break;
        default:
            m__raw_body = m__io->read_bytes((length() - 4));
            break;
        }
    }
}

mach_o_t::segment_command_64_t::section_64_t::eh_frame_item_t::~eh_frame_item_t() {
}

mach_o_t::segment_command_64_t::section_64_t::eh_frame_item_t::char_chain_t::char_chain_t(kaitai::kstream *p_io, kaitai::kstruct *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_chr = m__io->read_u1();
    n_next = true;
    if (chr() != 0) {
        n_next = false;
        m_next = new char_chain_t(m__io, this, m__root);
    }
}

mach_o_t::segment_command_64_t::section_64_t::eh_frame_item_t::char_chain_t::~char_chain_t() {
    if (!n_next) {
        delete m_next;
    }
}

mach_o_t::segment_command_64_t::section_64_t::eh_frame_item_t::cie_t::cie_t(kaitai::kstream *p_io, mach_o_t::segment_command_64_t::section_64_t::eh_frame_item_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_version = m__io->read_u1();
    m_aug_str = new char_chain_t(m__io, this, m__root);
    m_code_alignment_factor = new uleb128_t(m__io, this, m__root);
    m_data_alignment_factor = new uleb128_t(m__io, this, m__root);
    m_return_address_register = m__io->read_u1();
    n_augmentation = true;
    if (aug_str()->chr() == 122) {
        n_augmentation = false;
        m_augmentation = new augmentation_entry_t(m__io, this, m__root);
    }
}

mach_o_t::segment_command_64_t::section_64_t::eh_frame_item_t::cie_t::~cie_t() {
    delete m_aug_str;
    delete m_code_alignment_factor;
    delete m_data_alignment_factor;
    if (!n_augmentation) {
        delete m_augmentation;
    }
}

mach_o_t::segment_command_64_t::section_64_t::eh_frame_item_t::augmentation_entry_t::augmentation_entry_t(kaitai::kstream *p_io, mach_o_t::segment_command_64_t::section_64_t::eh_frame_item_t::cie_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_length = new uleb128_t(m__io, this, m__root);
    n_fde_pointer_encoding = true;
    if (_parent()->aug_str()->next()->chr() == 82) {
        n_fde_pointer_encoding = false;
        m_fde_pointer_encoding = m__io->read_u1();
    }
}

mach_o_t::segment_command_64_t::section_64_t::eh_frame_item_t::augmentation_entry_t::~augmentation_entry_t() {
    delete m_length;
}

mach_o_t::segment_command_64_t::section_64_t::eh_frame_t::eh_frame_t(kaitai::kstream *p_io, mach_o_t::segment_command_64_t::section_64_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_items = new std::vector<eh_frame_item_t*>();
    while (!m__io->is_eof()) {
        m_items->push_back(new eh_frame_item_t(m__io, this, m__root));
    }
}

mach_o_t::segment_command_64_t::section_64_t::eh_frame_t::~eh_frame_t() {
    for (std::vector<eh_frame_item_t*>::iterator it = m_items->begin(); it != m_items->end(); ++it) {
        delete *it;
    }
    delete m_items;
}

mach_o_t::segment_command_64_t::section_64_t::pointer_list_t::pointer_list_t(kaitai::kstream *p_io, mach_o_t::segment_command_64_t::section_64_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_items = new std::vector<uint64_t>();
    while (!m__io->is_eof()) {
        m_items->push_back(m__io->read_u8le());
    }
}

mach_o_t::segment_command_64_t::section_64_t::pointer_list_t::~pointer_list_t() {
    delete m_items;
}

mach_o_t::segment_command_64_t::section_64_t::string_list_t::string_list_t(kaitai::kstream *p_io, mach_o_t::segment_command_64_t::section_64_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_strings = new std::vector<std::string>();
    while (!m__io->is_eof()) {
        m_strings->push_back(m__io->read_strz("ascii", 0, false, true, true));
    }
}

mach_o_t::segment_command_64_t::section_64_t::string_list_t::~string_list_t() {
    delete m_strings;
}

kaitai::kstruct* mach_o_t::segment_command_64_t::section_64_t::data() {
    if (f_data)
        return m_data;
    kaitai::kstream *io = _root()->_io();
    std::streampos _pos = io->pos();
    io->seek(offset());
    {
        std::string on = sect_name();
        if (on == std::string("__objc_methname ")) {
            m__raw_data = io->read_bytes(size());
            m__io__raw_data = new kaitai::kstream(m__raw_data);
            m_data = new string_list_t(m__io__raw_data, this, m__root);
        }
        else if (on == std::string("__objc_nlclslist")) {
            m__raw_data = io->read_bytes(size());
            m__io__raw_data = new kaitai::kstream(m__raw_data);
            m_data = new pointer_list_t(m__io__raw_data, this, m__root);
        }
        else if (on == std::string("__eh_frame      ")) {
            m__raw_data = io->read_bytes(size());
            m__io__raw_data = new kaitai::kstream(m__raw_data);
            m_data = new eh_frame_t(m__io__raw_data, this, m__root);
        }
        else if (on == std::string("__objc_classlist")) {
            m__raw_data = io->read_bytes(size());
            m__io__raw_data = new kaitai::kstream(m__raw_data);
            m_data = new pointer_list_t(m__io__raw_data, this, m__root);
        }
        else if (on == std::string("__cstring       ")) {
            m__raw_data = io->read_bytes(size());
            m__io__raw_data = new kaitai::kstream(m__raw_data);
            m_data = new string_list_t(m__io__raw_data, this, m__root);
        }
        else if (on == std::string("__objc_methtype ")) {
            m__raw_data = io->read_bytes(size());
            m__io__raw_data = new kaitai::kstream(m__raw_data);
            m_data = new string_list_t(m__io__raw_data, this, m__root);
        }
        else if (on == std::string("__objc_protolist")) {
            m__raw_data = io->read_bytes(size());
            m__io__raw_data = new kaitai::kstream(m__raw_data);
            m_data = new pointer_list_t(m__io__raw_data, this, m__root);
        }
        else if (on == std::string("__la_symbol_ptr ")) {
            m__raw_data = io->read_bytes(size());
            m__io__raw_data = new kaitai::kstream(m__raw_data);
            m_data = new pointer_list_t(m__io__raw_data, this, m__root);
        }
        else if (on == std::string("__objc_imageinfo")) {
            m__raw_data = io->read_bytes(size());
            m__io__raw_data = new kaitai::kstream(m__raw_data);
            m_data = new pointer_list_t(m__io__raw_data, this, m__root);
        }
        else if (on == std::string("__objc_selrefs  ")) {
            m__raw_data = io->read_bytes(size());
            m__io__raw_data = new kaitai::kstream(m__raw_data);
            m_data = new pointer_list_t(m__io__raw_data, this, m__root);
        }
        else if (on == std::string("__cfstring      ")) {
            m__raw_data = io->read_bytes(size());
            m__io__raw_data = new kaitai::kstream(m__raw_data);
            m_data = new cf_string_list_t(m__io__raw_data, this, m__root);
        }
        else if (on == std::string("__objc_classrefs")) {
            m__raw_data = io->read_bytes(size());
            m__io__raw_data = new kaitai::kstream(m__raw_data);
            m_data = new pointer_list_t(m__io__raw_data, this, m__root);
        }
        else if (on == std::string("__objc_protorefs")) {
            m__raw_data = io->read_bytes(size());
            m__io__raw_data = new kaitai::kstream(m__raw_data);
            m_data = new pointer_list_t(m__io__raw_data, this, m__root);
        }
        else if (on == std::string("__got           ")) {
            m__raw_data = io->read_bytes(size());
            m__io__raw_data = new kaitai::kstream(m__raw_data);
            m_data = new pointer_list_t(m__io__raw_data, this, m__root);
        }
        else if (on == std::string("__objc_classname")) {
            m__raw_data = io->read_bytes(size());
            m__io__raw_data = new kaitai::kstream(m__raw_data);
            m_data = new string_list_t(m__io__raw_data, this, m__root);
        }
        else if (on == std::string("__nl_symbol_ptr ")) {
            m__raw_data = io->read_bytes(size());
            m__io__raw_data = new kaitai::kstream(m__raw_data);
            m_data = new pointer_list_t(m__io__raw_data, this, m__root);
        }
        else if (on == std::string("__objc_superrefs")) {
            m__raw_data = io->read_bytes(size());
            m__io__raw_data = new kaitai::kstream(m__raw_data);
            m_data = new pointer_list_t(m__io__raw_data, this, m__root);
        }
        else {
            m__raw_data = io->read_bytes(size());
        }
    }
    io->seek(_pos);
    f_data = true;
    return m_data;
}

mach_o_t::dysymtab_command_t::dysymtab_command_t(kaitai::kstream *p_io, mach_o_t::load_command_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    f_indirect_symbols = false;
    veles_obj = m__io->startChunk("dysymtab_command");
    m__io->pushName("i_local_sym");
    m_i_local_sym = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("n_local_sym");
    m_n_local_sym = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("i_ext_def_sym");
    m_i_ext_def_sym = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("n_ext_def_sym");
    m_n_ext_def_sym = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("i_undef_sym");
    m_i_undef_sym = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("n_undef_sym");
    m_n_undef_sym = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("toc_off");
    m_toc_off = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("n_toc");
    m_n_toc = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("mod_tab_off");
    m_mod_tab_off = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("n_mod_tab");
    m_n_mod_tab = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("ext_ref_sym_off");
    m_ext_ref_sym_off = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("n_ext_ref_syms");
    m_n_ext_ref_syms = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("indirect_sym_off");
    m_indirect_sym_off = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("n_indirect_syms");
    m_n_indirect_syms = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("ext_rel_off");
    m_ext_rel_off = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("n_ext_rel");
    m_n_ext_rel = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("loc_rel_off");
    m_loc_rel_off = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("n_loc_rel");
    m_n_loc_rel = m__io->read_u4le();
    m__io->popName();
    m__io->endChunk();
}

mach_o_t::dysymtab_command_t::~dysymtab_command_t() {
    if (f_indirect_symbols) {
        delete m_indirect_symbols;
    }
}

std::vector<uint32_t>* mach_o_t::dysymtab_command_t::indirect_symbols() {
    if (f_indirect_symbols)
        return m_indirect_symbols;
    kaitai::kstream *io = _root()->_io();
    std::streampos _pos = io->pos();
    io->seek(indirect_sym_off());
    int l_indirect_symbols = n_indirect_syms();
    m_indirect_symbols = new std::vector<uint32_t>();
    m_indirect_symbols->reserve(l_indirect_symbols);
    for (int i = 0; i < l_indirect_symbols; i++) {
        m_indirect_symbols->push_back(io->read_u4le());
    }
    io->seek(_pos);
    f_indirect_symbols = true;
    return m_indirect_symbols;
}

mach_o_t::mach_header_t::mach_header_t(kaitai::kstream *p_io, mach_o_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    veles_obj = m__io->startChunk("mach_header");
    m__io->pushName("cputype");
    m_cputype = static_cast<mach_o_t::cpu_type_t>(m__io->read_u4le());
    m__io->popName();
    m__io->pushName("cpusubtype");
    m_cpusubtype = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("filetype");
    m_filetype = static_cast<mach_o_t::file_type_t>(m__io->read_u4le());
    m__io->popName();
    m__io->pushName("ncmds");
    m_ncmds = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("sizeofcmds");
    m_sizeofcmds = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("flags");
    m_flags = m__io->read_u4le();
    m__io->popName();
    n_reserved = true;
    if (_root()->magic() == MAGIC_TYPE_MACHO_BE_X64 || _root()->magic() == MAGIC_TYPE_MACHO_LE_X64) {
        n_reserved = false;
        m__io->pushName("reserved");
        m_reserved = m__io->read_u4le();
        m__io->popName();
    }
    m__io->endChunk();
}

mach_o_t::mach_header_t::~mach_header_t() {
}

mach_o_t::linkedit_data_command_t::linkedit_data_command_t(kaitai::kstream *p_io, mach_o_t::load_command_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    veles_obj = m__io->startChunk("linkedit_data_command");
    m__io->pushName("data_off");
    m_data_off = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("data_size");
    m_data_size = m__io->read_u4le();
    m__io->popName();
    m__io->endChunk();
}

mach_o_t::linkedit_data_command_t::~linkedit_data_command_t() {
}

mach_o_t::version_t::version_t(kaitai::kstream *p_io, mach_o_t::version_min_command_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    veles_obj = m__io->startChunk("version");
    m__io->pushName("p1");
    m_p1 = m__io->read_u1();
    m__io->popName();
    m__io->pushName("minor");
    m_minor = m__io->read_u1();
    m__io->popName();
    m__io->pushName("major");
    m_major = m__io->read_u1();
    m__io->popName();
    m__io->pushName("release");
    m_release = m__io->read_u1();
    m__io->popName();
    m__io->endChunk();
}

mach_o_t::version_t::~version_t() {
}

mach_o_t::code_signature_command_t::code_signature_command_t(kaitai::kstream *p_io, mach_o_t::load_command_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    f_code_signature = false;
    veles_obj = m__io->startChunk("code_signature_command");
    m__io->pushName("data_off");
    m_data_off = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("data_size");
    m_data_size = m__io->read_u4le();
    m__io->popName();
    m__io->endChunk();
}

mach_o_t::code_signature_command_t::~code_signature_command_t() {
    if (f_code_signature) {
        delete m__io__raw_code_signature;
        delete m_code_signature;
    }
}

mach_o_t::cs_blob_t* mach_o_t::code_signature_command_t::code_signature() {
    if (f_code_signature)
        return m_code_signature;
    kaitai::kstream *io = _root()->_io();
    std::streampos _pos = io->pos();
    io->seek(data_off());
    m__raw_code_signature = io->read_bytes(data_size());
    m__io__raw_code_signature = new kaitai::kstream(m__raw_code_signature);
    m_code_signature = new cs_blob_t(m__io__raw_code_signature, this, m__root);
    io->seek(_pos);
    f_code_signature = true;
    return m_code_signature;
}

mach_o_t::dyld_info_command_t::dyld_info_command_t(kaitai::kstream *p_io, mach_o_t::load_command_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    f_rebase = false;
    f_bind = false;
    f_lazy_bind = false;
    f_exports = false;
    veles_obj = m__io->startChunk("dyld_info_command");
    m__io->pushName("rebase_off");
    m_rebase_off = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("rebase_size");
    m_rebase_size = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("bind_off");
    m_bind_off = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("bind_size");
    m_bind_size = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("weak_bind_off");
    m_weak_bind_off = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("weak_bind_size");
    m_weak_bind_size = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("lazy_bind_off");
    m_lazy_bind_off = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("lazy_bind_size");
    m_lazy_bind_size = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("export_off");
    m_export_off = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("export_size");
    m_export_size = m__io->read_u4le();
    m__io->popName();
    m__io->endChunk();
}

mach_o_t::dyld_info_command_t::~dyld_info_command_t() {
    if (f_rebase) {
        delete m__io__raw_rebase;
        delete m_rebase;
    }
    if (f_bind) {
        delete m__io__raw_bind;
        delete m_bind;
    }
    if (f_lazy_bind) {
        delete m__io__raw_lazy_bind;
        delete m_lazy_bind;
    }
    if (f_exports) {
        delete m__io__raw_exports;
        delete m_exports;
    }
}

mach_o_t::dyld_info_command_t::bind_item_t::bind_item_t(kaitai::kstream *p_io, kaitai::kstruct *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    f_opcode = false;
    f_immediate = false;
    m_opcode_and_immediate = m__io->read_u1();
    n_uleb = true;
    if (opcode() == BIND_OPCODE_SET_DYLIB_ORDINAL_ULEB || opcode() == BIND_OPCODE_SET_APPEND_SLEB || opcode() == BIND_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB || opcode() == BIND_OPCODE_ADD_ADDRESS_ULEB || opcode() == BIND_OPCODE_DO_BIND_ADD_ADDRESS_ULEB || opcode() == BIND_OPCODE_DO_BIND_ULEB_TIMES_SKIPPING_ULEB) {
        n_uleb = false;
        m_uleb = new uleb128_t(m__io, this, m__root);
    }
    n_skip = true;
    if (opcode() == BIND_OPCODE_DO_BIND_ULEB_TIMES_SKIPPING_ULEB) {
        n_skip = false;
        m_skip = new uleb128_t(m__io, this, m__root);
    }
    n_symbol = true;
    if (opcode() == BIND_OPCODE_SET_SYMBOL_TRAILING_FLAGS_IMMEDIATE) {
        n_symbol = false;
        m_symbol = m__io->read_strz("ascii", 0, false, true, true);
    }
}

mach_o_t::dyld_info_command_t::bind_item_t::~bind_item_t() {
    if (!n_uleb) {
        delete m_uleb;
    }
    if (!n_skip) {
        delete m_skip;
    }
}

mach_o_t::dyld_info_command_t::bind_opcode_t mach_o_t::dyld_info_command_t::bind_item_t::opcode() {
    if (f_opcode)
        return m_opcode;
    m_opcode = static_cast<mach_o_t::dyld_info_command_t::bind_opcode_t>((opcode_and_immediate() & 240));
    f_opcode = true;
    return m_opcode;
}

int32_t mach_o_t::dyld_info_command_t::bind_item_t::immediate() {
    if (f_immediate)
        return m_immediate;
    m_immediate = (opcode_and_immediate() & 15);
    f_immediate = true;
    return m_immediate;
}

mach_o_t::dyld_info_command_t::rebase_data_t::rebase_data_t(kaitai::kstream *p_io, mach_o_t::dyld_info_command_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_items = new std::vector<rebase_item_t*>();
    {
        rebase_item_t* _;
        do {
            _ = new rebase_item_t(m__io, this, m__root);
            m_items->push_back(_);
        } while (!(_->opcode() == OPCODE_DONE));
    }
}

mach_o_t::dyld_info_command_t::rebase_data_t::~rebase_data_t() {
    for (std::vector<rebase_item_t*>::iterator it = m_items->begin(); it != m_items->end(); ++it) {
        delete *it;
    }
    delete m_items;
}

mach_o_t::dyld_info_command_t::rebase_data_t::rebase_item_t::rebase_item_t(kaitai::kstream *p_io, mach_o_t::dyld_info_command_t::rebase_data_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    f_opcode = false;
    f_immediate = false;
    m_opcode_and_immediate = m__io->read_u1();
    n_uleb = true;
    if (opcode() == OPCODE_SET_SEGMENT_AND_OFFSET_ULEB || opcode() == OPCODE_ADD_ADDRESS_ULEB || opcode() == OPCODE_DO_REBASE_ULEB_TIMES || opcode() == OPCODE_DO_REBASE_ADD_ADDRESS_ULEB || opcode() == OPCODE_DO_REBASE_ULEB_TIMES_SKIPPING_ULEB) {
        n_uleb = false;
        m_uleb = new uleb128_t(m__io, this, m__root);
    }
    n_skip = true;
    if (opcode() == OPCODE_DO_REBASE_ULEB_TIMES_SKIPPING_ULEB) {
        n_skip = false;
        m_skip = new uleb128_t(m__io, this, m__root);
    }
}

mach_o_t::dyld_info_command_t::rebase_data_t::rebase_item_t::~rebase_item_t() {
    if (!n_uleb) {
        delete m_uleb;
    }
    if (!n_skip) {
        delete m_skip;
    }
}

mach_o_t::dyld_info_command_t::rebase_data_t::opcode_t mach_o_t::dyld_info_command_t::rebase_data_t::rebase_item_t::opcode() {
    if (f_opcode)
        return m_opcode;
    m_opcode = static_cast<mach_o_t::dyld_info_command_t::rebase_data_t::opcode_t>((opcode_and_immediate() & 240));
    f_opcode = true;
    return m_opcode;
}

int32_t mach_o_t::dyld_info_command_t::rebase_data_t::rebase_item_t::immediate() {
    if (f_immediate)
        return m_immediate;
    m_immediate = (opcode_and_immediate() & 15);
    f_immediate = true;
    return m_immediate;
}

mach_o_t::dyld_info_command_t::export_node_t::export_node_t(kaitai::kstream *p_io, kaitai::kstruct *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_terminal_size = new uleb128_t(m__io, this, m__root);
    m_children_count = m__io->read_u1();
    int l_children = children_count();
    m_children = new std::vector<child_t*>();
    m_children->reserve(l_children);
    for (int i = 0; i < l_children; i++) {
        m_children->push_back(new child_t(m__io, this, m__root));
    }
    m_terminal = m__io->read_bytes(terminal_size()->value());
}

mach_o_t::dyld_info_command_t::export_node_t::~export_node_t() {
    delete m_terminal_size;
    for (std::vector<child_t*>::iterator it = m_children->begin(); it != m_children->end(); ++it) {
        delete *it;
    }
    delete m_children;
}

mach_o_t::dyld_info_command_t::export_node_t::child_t::child_t(kaitai::kstream *p_io, mach_o_t::dyld_info_command_t::export_node_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    f_value = false;
    m_name = m__io->read_strz("ascii", 0, false, true, true);
    m_node_offset = new uleb128_t(m__io, this, m__root);
}

mach_o_t::dyld_info_command_t::export_node_t::child_t::~child_t() {
    delete m_node_offset;
    if (f_value) {
        delete m_value;
    }
}

mach_o_t::dyld_info_command_t::export_node_t* mach_o_t::dyld_info_command_t::export_node_t::child_t::value() {
    if (f_value)
        return m_value;
    std::streampos _pos = m__io->pos();
    m__io->seek(node_offset()->value());
    m_value = new export_node_t(m__io, this, m__root);
    m__io->seek(_pos);
    f_value = true;
    return m_value;
}

mach_o_t::dyld_info_command_t::bind_data_t::bind_data_t(kaitai::kstream *p_io, mach_o_t::dyld_info_command_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_items = new std::vector<bind_item_t*>();
    {
        bind_item_t* _;
        do {
            _ = new bind_item_t(m__io, this, m__root);
            m_items->push_back(_);
        } while (!(_->opcode() == BIND_OPCODE_DONE));
    }
}

mach_o_t::dyld_info_command_t::bind_data_t::~bind_data_t() {
    for (std::vector<bind_item_t*>::iterator it = m_items->begin(); it != m_items->end(); ++it) {
        delete *it;
    }
    delete m_items;
}

mach_o_t::dyld_info_command_t::lazy_bind_data_t::lazy_bind_data_t(kaitai::kstream *p_io, mach_o_t::dyld_info_command_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_items = new std::vector<bind_item_t*>();
    while (!m__io->is_eof()) {
        m_items->push_back(new bind_item_t(m__io, this, m__root));
    }
}

mach_o_t::dyld_info_command_t::lazy_bind_data_t::~lazy_bind_data_t() {
    for (std::vector<bind_item_t*>::iterator it = m_items->begin(); it != m_items->end(); ++it) {
        delete *it;
    }
    delete m_items;
}

mach_o_t::dyld_info_command_t::rebase_data_t* mach_o_t::dyld_info_command_t::rebase() {
    if (f_rebase)
        return m_rebase;
    kaitai::kstream *io = _root()->_io();
    std::streampos _pos = io->pos();
    io->seek(rebase_off());
    m__raw_rebase = io->read_bytes(rebase_size());
    m__io__raw_rebase = new kaitai::kstream(m__raw_rebase);
    m_rebase = new rebase_data_t(m__io__raw_rebase, this, m__root);
    io->seek(_pos);
    f_rebase = true;
    return m_rebase;
}

mach_o_t::dyld_info_command_t::bind_data_t* mach_o_t::dyld_info_command_t::bind() {
    if (f_bind)
        return m_bind;
    kaitai::kstream *io = _root()->_io();
    std::streampos _pos = io->pos();
    io->seek(bind_off());
    m__raw_bind = io->read_bytes(bind_size());
    m__io__raw_bind = new kaitai::kstream(m__raw_bind);
    m_bind = new bind_data_t(m__io__raw_bind, this, m__root);
    io->seek(_pos);
    f_bind = true;
    return m_bind;
}

mach_o_t::dyld_info_command_t::lazy_bind_data_t* mach_o_t::dyld_info_command_t::lazy_bind() {
    if (f_lazy_bind)
        return m_lazy_bind;
    kaitai::kstream *io = _root()->_io();
    std::streampos _pos = io->pos();
    io->seek(lazy_bind_off());
    m__raw_lazy_bind = io->read_bytes(lazy_bind_size());
    m__io__raw_lazy_bind = new kaitai::kstream(m__raw_lazy_bind);
    m_lazy_bind = new lazy_bind_data_t(m__io__raw_lazy_bind, this, m__root);
    io->seek(_pos);
    f_lazy_bind = true;
    return m_lazy_bind;
}

mach_o_t::dyld_info_command_t::export_node_t* mach_o_t::dyld_info_command_t::exports() {
    if (f_exports)
        return m_exports;
    kaitai::kstream *io = _root()->_io();
    std::streampos _pos = io->pos();
    io->seek(export_off());
    m__raw_exports = io->read_bytes(export_size());
    m__io__raw_exports = new kaitai::kstream(m__raw_exports);
    m_exports = new export_node_t(m__io__raw_exports, this, m__root);
    io->seek(_pos);
    f_exports = true;
    return m_exports;
}

mach_o_t::dylinker_command_t::dylinker_command_t(kaitai::kstream *p_io, mach_o_t::load_command_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    veles_obj = m__io->startChunk("dylinker_command");
    m_name = new lc_str_t(m__io, this, m__root);
    m__io->endChunk();
}

mach_o_t::dylinker_command_t::~dylinker_command_t() {
    delete m_name;
}

mach_o_t::dylib_command_t::dylib_command_t(kaitai::kstream *p_io, mach_o_t::load_command_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    veles_obj = m__io->startChunk("dylib_command");
    m__io->pushName("name_offset");
    m_name_offset = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("timestamp");
    m_timestamp = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("current_version");
    m_current_version = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("compatibility_version");
    m_compatibility_version = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("name");
    m_name = m__io->read_strz("utf-8", 0, false, true, true);
    m__io->popName();
    m__io->endChunk();
}

mach_o_t::dylib_command_t::~dylib_command_t() {
}

mach_o_t::lc_str_t::lc_str_t(kaitai::kstream *p_io, mach_o_t::dylinker_command_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    veles_obj = m__io->startChunk("lc_str");
    m__io->pushName("length");
    m_length = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("value");
    m_value = m__io->read_strz("UTF-8", 0, false, true, true);
    m__io->popName();
    m__io->endChunk();
}

mach_o_t::lc_str_t::~lc_str_t() {
}

mach_o_t::load_command_t::load_command_t(kaitai::kstream *p_io, mach_o_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_body = nullptr;
    m__io__raw_body = nullptr;
    veles_obj = m__io->startChunk("load_command");
    m__io->pushName("type");
    m_type = static_cast<mach_o_t::load_command_type_t>(m__io->read_u4le());
    m__io->popName();
    m__io->pushName("size");
    m_size = m__io->read_u4le();
    m__io->popName();
    uint64_t body_start = m__io->pos();
    switch (type()) {
    case LOAD_COMMAND_TYPE_RPATH: {
        m__io->pushName("_skip_me_body");
        m__raw_body = m__io->read_bytes(size() - 8);
        m__io->popName();
        m__io__raw_body = new kaitai::kstream(m__io->blob(), body_start, veles_obj, size() - 8);
        m_body = new rpath_command_t(m__io__raw_body, this, m__root);
        m__io->addSubchunkItem(body_start, body_start + (size() - 8), "body", static_cast<rpath_command_t*>(m_body)->veles_obj);
        break;
    }
    case LOAD_COMMAND_TYPE_SOURCE_VERSION: {
        m__io->pushName("_skip_me_body");
        m__raw_body = m__io->read_bytes(size() - 8);
        m__io->popName();
        m__io__raw_body = new kaitai::kstream(m__io->blob(), body_start, veles_obj, size() - 8);
        m_body = new source_version_command_t(m__io__raw_body, this, m__root);
        m__io->addSubchunkItem(body_start, body_start + (size() - 8), "body", static_cast<source_version_command_t*>(m_body)->veles_obj);
        break;
    }
    case LOAD_COMMAND_TYPE_LOAD_DYLINKER: {
        m__io->pushName("_skip_me_body");
        m__raw_body = m__io->read_bytes(size() - 8);
        m__io->popName();
        m__io__raw_body = new kaitai::kstream(m__io->blob(), body_start, veles_obj, size() - 8);
        m_body = new dylinker_command_t(m__io__raw_body, this, m__root);
        m__io->addSubchunkItem(body_start, body_start + (size() - 8), "body", static_cast<dylinker_command_t*>(m_body)->veles_obj);
        break;
    }
    case LOAD_COMMAND_TYPE_SYMTAB: {
        m__io->pushName("_skip_me_body");
        m__raw_body = m__io->read_bytes(size() - 8);
        m__io->popName();
        m__io__raw_body = new kaitai::kstream(m__io->blob(), body_start, veles_obj, size() - 8);
        m_body = new symtab_command_t(m__io__raw_body, this, m__root);
        m__io->addSubchunkItem(body_start, body_start + (size() - 8), "body", static_cast<symtab_command_t*>(m_body)->veles_obj);
        break;
    }
    case LOAD_COMMAND_TYPE_MAIN: {
        m__io->pushName("_skip_me_body");
        m__raw_body = m__io->read_bytes(size() - 8);
        m__io->popName();
        m__io__raw_body = new kaitai::kstream(m__io->blob(), body_start, veles_obj, size() - 8);
        m_body = new entry_point_command_t(m__io__raw_body, this, m__root);
        m__io->addSubchunkItem(body_start, body_start + (size() - 8), "body", static_cast<entry_point_command_t*>(m_body)->veles_obj);
        break;
    }
    case LOAD_COMMAND_TYPE_FUNCTION_STARTS: {
        m__io->pushName("_skip_me_body");
        m__raw_body = m__io->read_bytes(size() - 8);
        m__io->popName();
        m__io__raw_body = new kaitai::kstream(m__io->blob(), body_start, veles_obj, size() - 8);
        m_body = new linkedit_data_command_t(m__io__raw_body, this, m__root);
        m__io->addSubchunkItem(body_start, body_start + (size() - 8), "body", static_cast<linkedit_data_command_t*>(m_body)->veles_obj);
        break;
    }
    case LOAD_COMMAND_TYPE_VERSION_MIN_MACOSX: {
        m__io->pushName("_skip_me_body");
        m__raw_body = m__io->read_bytes(size() - 8);
        m__io->popName();
        m__io__raw_body = new kaitai::kstream(m__io->blob(), body_start, veles_obj, size() - 8);
        m_body = new version_min_command_t(m__io__raw_body, this, m__root);
        m__io->addSubchunkItem(body_start, body_start + (size() - 8), "body", static_cast<version_min_command_t*>(m_body)->veles_obj);
        break;
    }
    case LOAD_COMMAND_TYPE_DATA_IN_CODE: {
        m__io->pushName("_skip_me_body");
        m__raw_body = m__io->read_bytes(size() - 8);
        m__io->popName();
        m__io__raw_body = new kaitai::kstream(m__io->blob(), body_start, veles_obj, size() - 8);
        m_body = new linkedit_data_command_t(m__io__raw_body, this, m__root);
        m__io->addSubchunkItem(body_start, body_start + (size() - 8), "body", static_cast<linkedit_data_command_t*>(m_body)->veles_obj);
        break;
    }
    case LOAD_COMMAND_TYPE_UUID: {
        m__io->pushName("_skip_me_body");
        m__raw_body = m__io->read_bytes(size() - 8);
        m__io->popName();
        m__io__raw_body = new kaitai::kstream(m__io->blob(), body_start, veles_obj, size() - 8);
        m_body = new uuid_command_t(m__io__raw_body, this, m__root);
        m__io->addSubchunkItem(body_start, body_start + (size() - 8), "body", static_cast<uuid_command_t*>(m_body)->veles_obj);
        break;
    }
    case LOAD_COMMAND_TYPE_DYLD_INFO_ONLY: {
        m__io->pushName("_skip_me_body");
        m__raw_body = m__io->read_bytes(size() - 8);
        m__io->popName();
        m__io__raw_body = new kaitai::kstream(m__io->blob(), body_start, veles_obj, size() - 8);
        m_body = new dyld_info_command_t(m__io__raw_body, this, m__root);
        m__io->addSubchunkItem(body_start, body_start + (size() - 8), "body", static_cast<dyld_info_command_t*>(m_body)->veles_obj);
        break;
    }
    case LOAD_COMMAND_TYPE_CODE_SIGNATURE: {
        m__io->pushName("_skip_me_body");
        m__raw_body = m__io->read_bytes(size() - 8);
        m__io->popName();
        m__io__raw_body = new kaitai::kstream(m__io->blob(), body_start, veles_obj, size() - 8);
        m_body = new code_signature_command_t(m__io__raw_body, this, m__root);
        m__io->addSubchunkItem(body_start, body_start + (size() - 8), "body", static_cast<code_signature_command_t*>(m_body)->veles_obj);
        break;
    }
    case LOAD_COMMAND_TYPE_DYSYMTAB: {
        m__io->pushName("_skip_me_body");
        m__raw_body = m__io->read_bytes(size() - 8);
        m__io->popName();
        m__io__raw_body = new kaitai::kstream(m__io->blob(), body_start, veles_obj, size() - 8);
        m_body = new dysymtab_command_t(m__io__raw_body, this, m__root);
        m__io->addSubchunkItem(body_start, body_start + (size() - 8), "body", static_cast<dysymtab_command_t*>(m_body)->veles_obj);
        break;
    }
    case LOAD_COMMAND_TYPE_LOAD_DYLIB: {
        m__io->pushName("_skip_me_body");
        m__raw_body = m__io->read_bytes(size() - 8);
        m__io->popName();
        m__io__raw_body = new kaitai::kstream(m__io->blob(), body_start, veles_obj, size() - 8);
        m_body = new dylib_command_t(m__io__raw_body, this, m__root);
        m__io->addSubchunkItem(body_start, body_start + (size() - 8), "body", static_cast<dylib_command_t*>(m_body)->veles_obj);
        break;
    }
    case LOAD_COMMAND_TYPE_SEGMENT_64: {
        m__io->pushName("_skip_me_body");
        m__raw_body = m__io->read_bytes(size() - 8);
        m__io->popName();
        m__io__raw_body = new kaitai::kstream(m__io->blob(), body_start, veles_obj, size() - 8);
        m_body = new segment_command_64_t(m__io__raw_body, this, m__root);
        m__io->addSubchunkItem(body_start, body_start + (size() - 8), "body", static_cast<segment_command_64_t*>(m_body)->veles_obj);
        break;
    }
    default:
        m__io->pushName("body");
        m__raw_body = m__io->read_bytes(size() - 8);
        m__io->popName();
        break;
    }
    m__io->endChunk();
}

mach_o_t::load_command_t::~load_command_t() {
    delete m__io__raw_body;
    delete m_body;
}

mach_o_t::uuid_command_t::uuid_command_t(kaitai::kstream *p_io, mach_o_t::load_command_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    veles_obj = m__io->startChunk("uuid_command");
    m__io->pushName("uuid");
    m_uuid = m__io->read_bytes(16);
    m__io->popName();
    m__io->endChunk();
}

mach_o_t::uuid_command_t::~uuid_command_t() {
}

mach_o_t::symtab_command_t::symtab_command_t(kaitai::kstream *p_io, mach_o_t::load_command_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    f_symbols = false;
    f_strs = false;
    veles_obj = m__io->startChunk("symtab_command");
    m__io->pushName("sym_off");
    m_sym_off = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("n_syms");
    m_n_syms = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("str_off");
    m_str_off = m__io->read_u4le();
    m__io->popName();
    m__io->pushName("str_size");
    m_str_size = m__io->read_u4le();
    m__io->popName();
    m__io->endChunk();
}

mach_o_t::symtab_command_t::~symtab_command_t() {
    if (f_symbols) {
        for (std::vector<nlist_64_t*>::iterator it = m_symbols->begin(); it != m_symbols->end(); ++it) {
            delete *it;
        }
        delete m_symbols;
    }
    if (f_strs) {
        delete m__io__raw_strs;
        delete m_strs;
    }
}

mach_o_t::symtab_command_t::str_table_t::str_table_t(kaitai::kstream *p_io, mach_o_t::symtab_command_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_unknown = m__io->read_u4le();
    m_items = new std::vector<std::string>();
    {
        std::string _;
        do {
            _ = m__io->read_strz("ascii", 0, false, true, true);
            m_items->push_back(_);
        } while (!(_ == (std::string(""))));
    }
}

mach_o_t::symtab_command_t::str_table_t::~str_table_t() {
    delete m_items;
}

mach_o_t::symtab_command_t::nlist_64_t::nlist_64_t(kaitai::kstream *p_io, mach_o_t::symtab_command_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    m_un = m__io->read_u4le();
    m_type = m__io->read_u1();
    m_sect = m__io->read_u1();
    m_desc = m__io->read_u2le();
    m_value = m__io->read_u8le();
}

mach_o_t::symtab_command_t::nlist_64_t::~nlist_64_t() {
}

std::vector<mach_o_t::symtab_command_t::nlist_64_t*>* mach_o_t::symtab_command_t::symbols() {
    if (f_symbols)
        return m_symbols;
    kaitai::kstream *io = _root()->_io();
    std::streampos _pos = io->pos();
    io->seek(sym_off());
    int l_symbols = n_syms();
    m_symbols = new std::vector<nlist_64_t*>();
    m_symbols->reserve(l_symbols);
    for (int i = 0; i < l_symbols; i++) {
        m_symbols->push_back(new nlist_64_t(io, this, m__root));
    }
    io->seek(_pos);
    f_symbols = true;
    return m_symbols;
}

mach_o_t::symtab_command_t::str_table_t* mach_o_t::symtab_command_t::strs() {
    if (f_strs)
        return m_strs;
    kaitai::kstream *io = _root()->_io();
    std::streampos _pos = io->pos();
    io->seek(str_off());
    m__raw_strs = io->read_bytes(str_size());
    m__io__raw_strs = new kaitai::kstream(m__raw_strs);
    m_strs = new str_table_t(m__io__raw_strs, this, m__root);
    io->seek(_pos);
    f_strs = true;
    return m_strs;
}

mach_o_t::version_min_command_t::version_min_command_t(kaitai::kstream *p_io, mach_o_t::load_command_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    veles_obj = m__io->startChunk("version_min_command");
    m_version = new version_t(m__io, this, m__root);
    m_reserved = new version_t(m__io, this, m__root);
    m__io->endChunk();
}

mach_o_t::version_min_command_t::~version_min_command_t() {
    delete m_version;
    delete m_reserved;
}

mach_o_t::entry_point_command_t::entry_point_command_t(kaitai::kstream *p_io, mach_o_t::load_command_t *p_parent, mach_o_t *p_root) : kaitai::kstruct(p_io) {
    m__parent = p_parent;
    m__root = p_root;
    veles_obj = m__io->startChunk("entry_point_command");
    m__io->pushName("entry_off");
    m_entry_off = m__io->read_u8le();
    m__io->popName();
    m__io->pushName("stack_size");
    m_stack_size = m__io->read_u8le();
    m__io->popName();
    m__io->endChunk();
}

mach_o_t::entry_point_command_t::~entry_point_command_t() {
}
